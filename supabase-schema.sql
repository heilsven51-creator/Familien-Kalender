...
begin
  ...
  values (target_family_id, auth.uid(), lower(trim(invitee_email)), nullif(trim(invite_message), ''))
...
begin
  ...
  where id = invitation_id and status = 'pending' and lower(email) = lower(coalesce(auth.jwt() ->> 'email', ''))
  for update;
